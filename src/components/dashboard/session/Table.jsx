      ...
      <div className="py-6 px-4 sm:px-6 flex flex-col h-full gap-4 sm:gap-6">
        ...
        <div className="w-full themed-dashTable min-h-full bg-[#F6F6F6] rounded-3xl overflow-hidden">
          <div className="p-2 sm:p-4 overflow-auto">
            {/* Table Header */}
            <div className="bg-[#F1F1F1] themed-dashTable-header rounded-[16px] text-[#707070] p-3 grid grid-cols-4 sm:grid-cols-5 gap-2 sm:gap-4 text-xs sm:text-sm font-yekan-600">
              <p className="hidden sm:block">#</p>
              <p>عنوان جلسه</p>
              <p>عنوان تکلیف</p>
              <p className="hidden sm:block">نام گروه</p>
              <p>تاریخ</p>
            </div>
            ...
            <div className="overflow-y-auto">
              {filteredSessions.length === 0 ? (
                <p className="flex items-center justify-center py-8 sm:py-16 text-sm sm:text-base">
                ...
                filteredSessions.map((item, index) => (
                  <div
                    key={item.sessionId}
                    className="grid grid-cols-4 sm:grid-cols-5 gap-2 sm:gap-4 items-center py-3 sm:py-4 text-xs sm:text-sm text-black border-b border-gray-200"
                  >
                    <p className="hidden sm:block font-yekan-600">{index + 1}</p>
                    <p className="truncate font-yekan-600">{item.sessionTitle || '-'}</p>
                    <p className="truncate font-yekan-600">{item.hwTitle || '-'}</p>
                    <p className="hidden sm:block truncate font-yekan-600">{item.groupName || '-'}</p>
                    <p className="font-yekan-600">{formatDate(item.homeWorkDate)}</p>
                  </div>
                ))
              )}
            </div>

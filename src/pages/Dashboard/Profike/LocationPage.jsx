...
const LocationPage = () => {
  ...
  const handleLocationChange = (latlng) => {
    ...
    toast.success('موقعیت مکانی شما با موفقیت انتخاب شد.');
  };
  ...
  return (
    <div>
      ...
      {!showMap ? (
        ...
        <div className='w-full h-[404px] p-4 ml-3 rounded-2xl overflow-hidden'>
          <MapContainer
      ...
      {position && (
        <div className="mt-4 text-center text-[13px] text-gray-700">
          <p className="font-yekan-600 text-[#3772FF] mb-2">مختصات انتخاب شده:</p>
          <p>Lat: {position.lat.toFixed(5)}, Lng: {position.lng.toFixed(5)}</p>
          <p className="mt-2 font-yekan-500 text-black">📍 آدرس: {address || 'در حال دریافت آدرس...'}</p>
        </div>
      )}
    </div>
  );

...
export const postAddLikeComment = async (currentUserLikeId) => {
    try {
        const result = await http.post(`/Course/AddCourseCommentLike?CourseCommandId=${currentUserLikeId}`);
...
export const deleteLikeComment = async (currentUserLikeId) => {
    try {
        const result = await http.delete(`/Course/DeleteCourseCommentLike?CourseCommandId=${currentUserLikeId}`);
        return result;
